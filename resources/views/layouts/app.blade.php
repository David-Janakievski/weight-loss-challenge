        ...
        <nav class="border-b border-border bg-card/60 backdrop-blur sticky top-0 z-40">
            <div class="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
                <a href="{{ route('dashboard') }}" class="flex items-center gap-2.5 shrink-0 group">
                    <img src="{{ asset('images/logo.png') }}" alt="Слабеење Лето 2026"
                ...
                <button id="nav-toggle"
                    class="md:hidden p-2 rounded-lg text-gray-400 hover:text-white hover:bg-white/10 transition"
            ...
            <div id="mobile-menu" class="md:hidden flex-col border-t border-border bg-card px-4 py-3 gap-1 text-sm">
                <a href="{{ route('dashboard') }}"
                    class="block px-3 py-2 rounded-lg hover:bg-white/5 hover:text-gold">Контролна табла</a>
                <a href="{{ route('leaderboard') }}"
                    class="block px-3 py-2 rounded-lg hover:bg-white/5 hover:text-gold">Рангирање</a>
                <a href="{{ route('checkin.create') }}"
                    class="block px-3 py-2 rounded-lg hover:bg-white/5 hover:text-gold">Пријавување</a>
                @if (auth()->user()->is_admin)
                    <a href="{{ route('admin.dashboard') }}"
                        class="block px-3 py-2 rounded-lg hover:bg-white/5 hover:text-gold">Администратор</a>
                @endif
                <form method="POST" action="{{ route('logout') }}" class="mt-1">
                    @csrf
                    <button
                        class="w-full text-left px-3 py-2 rounded-lg text-red-400 hover:bg-white/5 hover:text-red-300">Одјави
                        се</button>
                </form>
            </div>
        </nav>
