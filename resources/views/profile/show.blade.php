...
<div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6 md:mb-8">
    ...
    <div class="card p-4 sm:p-5">
        ...
        <p class="text-xl sm:text-2xl font-bold mt-1 {{ $profileUser->percentLost() >= 0 ? 'text-loss' : 'text-gain' }}">
...
<div class="card p-5">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div class="text-center">
            <img src="{{ Storage::url($profileUser->starting_photo) }}" class="rounded-lg w-full h-28 sm:h-32 object-cover">
            <p class="text-xs text-gray-500 mt-1">Почеток</p>
        </div>
        @endif
        @foreach($profileUser->checkins as $c)
        <div class="text-center">
            <img src="{{ Storage::url($c->photo) }}" class="rounded-lg w-full h-28 sm:h-32 object-cover">
